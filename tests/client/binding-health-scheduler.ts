...
s.section("Test 4: give-up at the cap, then a fresh check starts a new episode");
{
	...
	f.sched.check(LEAF, "status-tick");
	s.check(f.timers.filter((t) => !t.cleared).length === 1, "a later check opens a new episode");
}
